...
/*
  ...
  The matrix is a vector whose first nrow elements correspond to 
  column 1, the next nrow to column 2, etc.  This setup allows trivial
  comparison of different columns via iterator rangees.

  The comparison of column i to column j is done via a call to
  std::mismatch, which is an STL routine:
  http://en.cppreference.com/w/cpp/algorithm/mismatch

  Note: it is be tempting to return an Rcpp::LogicalVector
